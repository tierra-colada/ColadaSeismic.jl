function prepare_JUDI_geometry_2D(
  ...
  for i in 1:nsrc
    ...
    # srcYCellVec[i] = [0]
    srcZCellVec[i] = -h5geom.getTraceHeader(h5opt.src_zkey, indCellVec[i][1], 1, h5geom.getLengthUnits(), "m")[:]

    # Set up receiver structure
    recXCellVec[i] = h5geom.getTraceHeader([h5opt.rec_xkey], indCellVec[i], [h5geom.getLengthUnits()], ["m"])[:]  # [:] converts matrix to vector
    # recYCellVec[i] = zeros(length(recXCellVec[i]))
    recZCellVec[i] = -h5geom.getTraceHeader([h5opt.rec_zkey], indCellVec[i], [h5geom.getLengthUnits()], ["m"])[:]
  end

  srcGeometry = Geometry(srcXCellVec, 0.0, srcZCellVec; dt=src_dt, t=src_t1)
  recGeometry = Geometry(recXCellVec, 0.0, recZCellVec; dt=rec_dt, t=rec_t1)
...
function prepare_JUDI_geometry_3D(
  ...
  for i in 1:nsrc
    ...
    # Set up receiver structure
    rec_xy = h5geom.getTraceHeader([h5opt.rec_xkey, h5opt.rec_ykey], indCellVec[i], [h5geom.getLengthUnits(), h5geom.getLengthUnits()], ["m", "m"])
